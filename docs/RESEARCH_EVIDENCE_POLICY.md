...
- 사용자가 입력한 completeness 숫자는 증거가 아니다. 원시 payload hash와 연결 heartbeat를
  포함한 외부 고정 `DataManifest`를 다시 검증해야 한다.
- 청산이 발생하지 않은 시간은 정상일 수 있으므로 liquidation 건수를 보간하거나 기대 건수로
  사용하지 않는다. 해당 스트림의 연결 heartbeat 완전성과 gap으로 관측 가능성을 판정한다.
- 캐리의 현물과 무기한 가격은 모두 Bybit의 별도 상품 provenance로 저장하며 다른 거래소의
  현물을 대신 사용하지 않는다.
