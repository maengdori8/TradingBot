...
`research/out`의 기존 funding·WFO 결과는 `legacy_non_evidence`다. 특히 기존 펀딩 v2의
메이커 WFO는 Sharpe -0.307, MDD 34.7%로 게이트를 통과하지 못했다. 새 승급 판정은
가설·코드·데이터·비용 스냅샷 해시가 모두 포함된 단일 파이프라인 결과만 사용한다.

공식 과거 API로 백필할 수 없는 liquidation과 orderbook은 자체 미래 수집분만 사용한다.
수집 공백은 0으로 대체하지 않으며, feed completeness 99% 미만 또는 15분 초과 미확인
gap이 있는 실행은 `insufficient_data`로 분류한다.
...
`python -m src.evidence_gate`는 외부에 고정한 `candidate_results.json`, 후보 행렬과
벤치마크를 다시 읽어 원시 거래·일별 수익으로 위 지표를 계산한다. 미통과 후보는 사유와
아티팩트를 남기되 실행 계약을 만들지 않고, 전 항목 통과 후보만 Demo 활성화 계약을 만든다.
