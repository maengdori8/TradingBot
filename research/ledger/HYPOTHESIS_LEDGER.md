...
BACKLOG_REMAINING: 0
ITER_COUNT: 8
CONSECUTIVE_REJECTS: 8
LAST_VERDICT: STOP
```
**■ 정직한 STOP (재확인).** 백로그 10 + 추가검증 H-011(델타중립 펀딩 캐리) = 11개 전부 처리:
REJECTED 9(A1·A2·A3·B1·C1·D1·C2·D2·**델타중립**) + BLOCKED 2(B2·B3). **PROMOTE 0.** 최종보고: research/reports/FINAL_REPORT.md.
**H-011이 STOP 결론을 강화**: 기존에 "미검증·tier E"였던 델타중립 펀딩 캐리를 실제 검증 → 전 가설 중 가장 근접
(메이커 검색구간 +0.044%)하나 홀드아웃 음수·게이트 미통과·테이커 음수. **펀딩 캐리는 실재하나 이 setup의 비용
(특히 현물 0.1% 레그)이 먹는다.** 결론: 견고한 OOS 엣지는 단순~중간 복잡도로 없음. 유일 잔존 경로 = tier E
(기관급 저비용/현물 레그 sub-0.1% 리베이트·VIP — 이걸 확보하면 델타중립 캐리가 가장 유력 후보).
...
## [H-011] 델타중립 펀딩 캐리 (저회전 헤지 수확)  (tier: B→E 경계, status: REJECTED — 가장 근접)

- **재개 사유**: STOP 후 사용자 "알아서" 지시로 신규 가설 등재. 기존엔 "델타중립=perp-paper 밖(tier E)"이라
  *미검증*이었음 — 현물가+펀딩 히스토리를 받아 **실제 검증**으로 전환(신규성 충족).
- **가설**: 펀딩 스프레드 실재(승률 80%). 방향성은 가격리스크에 죽음. **롱현물+숏퍼프 헤지 + 저회전(한 번
  진입→다수 8h 펀딩 수확→한 번 청산)으로 진입/청산비용(~0.3%)을 분산하면 비용 문턱을 넘는가?**
- **신규성(5축)**: 데이터=현물+펀딩 신규 / 메커니즘=헤지캐리(비방향) / 비용구조=저회전 분산.
- **사전등록(2 config)**: 진입 funding≥0.0001/8h, 청산<0.00002. base(즉시)+minhold21(7일 강제보유).
  비용 taker 0.31%/maker 0.20%(퍼프+현물 4레그). 성공=게이트+홀드아웃>0.
- **룩어헤드 감사**: 펀딩은 정산시점 이후만 수취, 가격은 funding ts 이하 최근봉. ✅
- **결과** (14심볼, 펀딩 2691×14):
  | config/비용 | 거래 | 평균보유 | 펀딩수확 | 순%/거래 | 홀드아웃 |
  |---|---|---|---|---|---|
  | base/maker | 2666 | 2.7일 | +0.087% | −0.100% | −0.18% |
  | minhold21/taker | 961 | 10.6일 | +0.229% | −0.066% | −0.247% |
  | **minhold21/maker** | 961 | 10.6일 | +0.229% | **+0.044%** | **−0.137%** |
  - 7일 강제보유로 비용분산 → 메이커 순이 음→**+0.044%(검색구간) 전환**. 그러나 stats_gate(N=853) 전부 미통과:
    최선도 DSR 0.0105 / Sharpe 0.063 < 천장 0.150 / 부트스트랩 CI **−0.0004~+0.0020(0 포함)** + **동결홀드아웃 −0.137%**.
- **판정**: **REJECT (전 가설 중 가장 근접).** 진단: **펀딩 캐리 실재**(10일 +0.23% 수확)이고 저회전이 비용을
  실제 분산해 **메이커 검색구간 +0.044% 도달** — 그러나 ①홀드아웃 음수 ②게이트 미통과(노이즈 안)
  ③봇 실제 테이커선 −0.066%(현물 0.1% 레그 킬러) ④메이커는 현물+퍼프 양쪽 메이커 전제(리테일 비현실).
  **함의: 비용만 더 낮추면(현물 sub-0.1%/리베이트/VIP) 넘는다 → tier E(기관급 저비용)가 유일 잔존 경로라는 최강 증거.**
- **재현**: `python -m research.dl_funding_spot --start 2024-01-01` →
  `python -m research.funding_dn --holdout-until 2026-04-16T00:00:00+00:00 --n-prior 850`
