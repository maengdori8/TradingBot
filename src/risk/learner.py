...
def fetch_closed_trades(
    conn: sqlite3.Connection, lookback: int = 0, since: str | None = None
) -> list[dict]:
    ...
    Args:
        ...
        since: epoch 시작(ISO) — 이 시각 이전 청산거래 제외 (전략 체제 변경 시
            구체제 거래가 신체제 학습을 오염시키지 않도록)
    ...
    where = "status IN ('SL','TP','manual')"
    params: tuple = ()
    if since:
        where += " AND exit_time >= ?"
        params = (since,)
    rows = conn.execute(
        f"SELECT {cols} FROM trades WHERE {where} ORDER BY exit_time ASC",
        params,
    ).fetchall()
...
def count_closed_with_r(conn: sqlite3.Connection, since: str | None = None) -> int:
    """lookback 무관 전체 청산거래(r_multiple 기록) 누계 — 쿨다운/게이트 기준.

    Args:
        conn: DB 연결
        since: epoch 시작(ISO) — 신체제 거래만 카운트
    """
    where = "status IN ('SL','TP','manual') AND r_multiple IS NOT NULL"
    params: tuple = ()
    if since:
        where += " AND exit_time >= ?"
        params = (since,)
...
def write_overlay(adj: dict, raw_cfg: dict, total_n: int) -> bool:
    ...
    # 부활 버그 방지: 기존 오버레이의 baseline이 현재 config 원본과 다르면
    # (= config가 손/배포로 변경됨) 구체제 학습값을 승계하지 않고 빈 값에서 재시작.
    # 그렇지 않으면 새 baseline과 함께 옛 값이 기록돼 무효화 가드가 무력화된다.
    new_baseline = _build_baseline(raw_cfg)
    if existing.get("baseline") and existing["baseline"] != new_baseline:
        logger.warning("baseline 변경 감지 — 구체제 학습값 폐기, 오버레이 재시작")
        existing = {}
...
def maybe_update(paper=None, notifier=None, dry_run: bool | None = None,
    ...
    epoch = learning.get("epoch_start")                     # 전략 체제 전환 시각 (구체제 거래 제외)
    trades = fetch_closed_trades(db_conn, lookback, since=epoch)   # 통계용
    total_n = _n_with_r(trades)
    cum_n = count_closed_with_r(db_conn, since=epoch)       # 게이트/쿨다운 기준(epoch 내 누계)
